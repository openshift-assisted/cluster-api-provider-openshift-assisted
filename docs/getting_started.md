...
| `ASSISTED_SERVICE_NAME` | Name of the Assisted Installer service. It is used to generate the ignition URL when set to be retrieved internally (`USE_INTERNAL_IMAGE_URL=true`) |
| `ASSISTED_INSTALLER_NAMESPACE` | Namespace where Assisted Installer is running .It is used to generate the ignition URL when set to be retrieved internally (`USE_INTERNAL_IMAGE_URL=true`). Defaults to `assisted-service` |
| `ASSISTED_CA_BUNDLE_NAMESPACE` | Namespace for Assisted Installer CA bundle. |
| `ASSISTED_CA_BUNDLE_RESOURCE` | Type of the resource holding the CA bundle (i.e. configmap/secret). Defaults to `secret` |
| `ASSISTED_CA_BUNDLE_NAME` | Name of the resource holding the CA bundle. Defaults to `assisted-installer-ca` |
| `ASSISTED_CA_BUNDLE_KEY` | Key in the resource holding the CA bundle. Defaults to `ca.crt` |
...
* DNS entries for the expected endpoints:
  ...
  - `api.<baseDomain>`: The internal API endpoint for the cluster.
...
```bash
clusterctl describe cluster --namespace test-capi test-multinode --show-conditions=all
````
