...
func (r *ClusterDeploymentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Migrate owner references to current API versions.
	// If migration occurred, requeue to reconcile with the updated object.
	migrated, err := util.MigrateOwnerReferences(ctx, r.Client, clusterDeployment)
	if err != nil {
		log.Error(err, "failed to migrate owner references on ClusterDeployment", "clusterDeployment", clusterDeployment.Name)
	}
	if migrated {
		return ctrl.Result{RequeueAfter: 0}, nil
	}

	// Check if this ClusterDeployment has the managed-by label
	labels := clusterDeployment.GetLabels()
	if labels == nil {
		log.V(logutil.DebugLevel).Info("cluster deployment has no labels")
		return ctrl.Result{}, nil
	}
	acpUID, exists := labels[managedByLabel]
	if !exists {
		log.V(logutil.DebugLevel).Info("cluster deployment is not managed by OpenshiftAssistedControlPlane")
		return ctrl.Result{}, nil
	}

	// Find the OpenshiftAssistedControlPlane with this UID
	acps := controlplanev1alpha3.OpenshiftAssistedControlPlaneList{}
	if err := r.List(ctx, &acps, client.InNamespace(clusterDeployment.Namespace)); err != nil {
		log.Error(err, "failed to list OpenshiftAssistedControlPlanes")
		return ctrl.Result{}, err
	}
	var acp *controlplanev1alpha3.OpenshiftAssistedControlPlane
	for i := range acps.Items {
		if string(acps.Items[i].UID) == acpUID {
			acp = &acps.Items[i]
			break
		}
	}
	if acp == nil {
		log.V(logutil.DebugLevel).Info("OpenshiftAssistedControlPlane not found for ClusterDeployment", "UID", acpUID)
		return ctrl.Result{}, nil
	}
...
func (r *ClusterDeploymentReconciler) ensureAgentClusterInstall(
	...
	mutate := func() error {
		...
		_ = controllerutil.SetOwnerReference(&oacp, aci, r.Scheme)
