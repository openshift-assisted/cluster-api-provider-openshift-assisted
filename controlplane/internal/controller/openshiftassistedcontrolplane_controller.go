...
func (r *OpenshiftAssistedControlPlaneReconciler) getOwnedClusterDeployment(ctx context.Context, oacp *controlplanev1alpha3.OpenshiftAssistedControlPlane) (*hivev1.ClusterDeployment, error) {
	// Check if there is a ClusterDeployment managed by this OACP (using managed-by label)
	cds := hivev1.ClusterDeploymentList{}
	if err := r.List(
		ctx,
		&cds,
		client.InNamespace(oacp.Namespace),
		client.MatchingLabels{managedByLabel: string(oacp.UID)},
	); err != nil {
		return nil, err
	}
	if len(cds.Items) == 0 {
		return nil, nil // fmt.Errorf("no ClusterDeployment found for OACP %s", oacp.Name)
	}
	return &cds.Items[0], nil
}
...
func (r *OpenshiftAssistedControlPlaneReconciler) handleDeletion(ctx context.Context, oacp *controlplanev1alpha3.OpenshiftAssistedControlPlane) error {
	...
	cd, err := r.getOwnedClusterDeployment(ctx, oacp)
	if err != nil {
		return err
	}
	if cd != nil {
		if err := r.Delete(ctx, cd); err != nil {
			return err
		}
	}
...
func (r *OpenshiftAssistedControlPlaneReconciler) computeDesiredMachine(oacp *controlplanev1alpha3.OpenshiftAssistedControlPlane, name string, cluster *clusterv1.Cluster, failureDomain string) *clusterv1.Machine {
	...
	// We intentionally don't use the map directly to ensure we don't modify the map in KCP.
...
func IndexClusterDeploymentByOwner(rawObj client.Object) []string {
	cd := rawObj.(*hivev1.ClusterDeployment)
...
func (r *OpenshiftAssistedControlPlaneReconciler) ensureClusterDeployment(
	...
	// if ClusterDeployment already exists, we just need to ensureit has the controllerRef

	if oacp.Spec.Config.ClusterName != "" {
		clusterName = oacp.Spec.Config.ClusterName
	}

	// Get cluster clusterName instead of reference to oacp clusterName
	...
	_, err = controllerutil.CreateOrPatch(ctx, r.Client, cd, func() error {
		if err := controllerutil.SetOwnerReference(oacp, cd, r.Scheme); err != nil {
			return err
		}
...
func (r *OpenshiftAssistedControlPlaneReconciler) reconcileReplicas(ctx context.Context, oacp *controlplanev1alpha3.OpenshiftAssistedControlPlane, cluster *clusterv1.Cluster) error {
	...
	// Migrate owner references on machines to current API versions
	for _, machine := range machines {
		if _, err := util.MigrateOwnerReferences(ctx, r.Client, machine); err != nil {
			log.Error(err, "failed to migrate owner references on Machine", "machine", machine.Name)
		}
	}
