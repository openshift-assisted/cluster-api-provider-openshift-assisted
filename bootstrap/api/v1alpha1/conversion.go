...
func (src *OpenshiftAssistedConfig) ConvertTo(dstRaw conversion.Hub) error {
	...
	// Migrate deprecated status references to annotations for the controller to process.
	// The controller will add managed-by labels to the referenced resources and clear these annotations.
	if dst.Annotations == nil {
		dst.Annotations = make(map[string]string)
	}
	if src.Status.InfraEnvRef != nil && src.Status.InfraEnvRef.Name != "" {
		dst.Annotations[bootstrapv1alpha2.MigrateInfraEnvRefAnnotation] = src.Status.InfraEnvRef.Name
		if src.Status.InfraEnvRef.Namespace != "" {
			dst.Annotations[bootstrapv1alpha2.MigrateInfraEnvRefNamespaceAnnotation] = src.Status.InfraEnvRef.Namespace
		}
	}
	if src.Status.AgentRef != nil && src.Status.AgentRef.Name != "" {
		dst.Annotations[bootstrapv1alpha2.MigrateAgentRefAnnotation] = src.Status.AgentRef.Name
	}
