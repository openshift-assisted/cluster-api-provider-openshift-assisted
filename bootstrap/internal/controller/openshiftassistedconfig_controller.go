...
func (r *OpenshiftAssistedConfigReconciler) ensureInfraEnv(ctx context.Context, config *bootstrapv1alpha2.OpenshiftAssistedConfig, machine *clusterv1.Machine, clusterDeployment *hivev1.ClusterDeployment) (*aiv1beta1.InfraEnv, error) {
	...
	r.setOwnerReferences(ctx, infraEnv, config, machine)
...
func (r *OpenshiftAssistedConfigReconciler) setOwnerReferences(ctx context.Context, infraEnv *aiv1beta1.InfraEnv, config *bootstrapv1alpha2.OpenshiftAssistedConfig, machine *clusterv1.Machine) {
	r.replaceOwnerReference(infraEnv, config)
	r.replaceOwnerReference(infraEnv, machine)
}

// replaceOwnerReference removes any existing owner reference with the same group+kind
// but different version, then sets the new owner reference.
func (r *OpenshiftAssistedConfigReconciler) replaceOwnerReference(obj, owner client.Object) {
	gvks, _, err := r.Scheme.ObjectKinds(owner)
