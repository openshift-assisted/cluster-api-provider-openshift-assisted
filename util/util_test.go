...
var _ = Describe("MigrateOwnerReferences", func() {
	var (
		ctx       context.Context
	...
	Context("when owner references have outdated API versions", func() {
		It("migrates Machine owner reference from v1beta1 to v1beta2", func() {
			...
			migrated, err := MigrateOwnerReferences(ctx, k8sClient, secret)
			...
			Expect(updatedSecret.OwnerReferences[0].APIVersion).To(Equal(clusterv1.GroupVersion.String()))
		})

		It("migrates Cluster owner reference from v1beta1 to v1beta2", func() {
			secret := &corev1.Secret{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-secret",
					Namespace: "default",
		...
		It("migrates OpenshiftAssistedConfig from v1alpha1 to v1alpha2", func() {
			secret := &corev1.Secret{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-secret",
					Namespace: "default",
			...
			updatedSecret := &corev1.Secret{}
			Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(secret), updatedSecret)).To(Succeed())
			Expect(updatedSecret.OwnerReferences[0].APIVersion).To(Equal("bootstrap.cluster.x-k8s.io/v1alpha2"))
		})

		It("migrates OpenshiftAssistedControlPlane from v1alpha2 to v1alpha3", func() {
			secret := &corev1.Secret{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-secret",
					Namespace: "default",
			...
			updatedSecret := &corev1.Secret{}
			Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(secret), updatedSecret)).To(Succeed())
			Expect(updatedSecret.OwnerReferences[0].APIVersion).To(Equal("controlplane.cluster.x-k8s.io/v1alpha3"))
		})

		It("migrates multiple owner references", func() {
			secret := &corev1.Secret{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-secret",
					Namespace: "default",
			...
			updatedSecret := &corev1.Secret{}
			Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(secret), updatedSecret)).To(Succeed())
			Expect(updatedSecret.OwnerReferences).To(HaveLen(2))
			Expect(updatedSecret.OwnerReferences[0].APIVersion).To(Equal(clusterv1.GroupVersion.String()))
			Expect(updatedSecret.OwnerReferences[1].APIVersion).To(Equal("controlplane.cluster.x-k8s.io/v1alpha3"))
		})
	})

	Context("when owner references are already current", func() {
		It("returns false without updating when Machine is already v1beta2", func() {
			secret := &corev1.Secret{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-secret",
					Namespace: "default",
		...
		It("returns false without updating when OpenshiftAssistedControlPlane is already v1alpha3", func() {
			secret := &corev1.Secret{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-secret",
					Namespace: "default",
	...
	Context("when owner references are not in the migration map", func() {
		It("skips unknown Kinds", func() {
			secret := &corev1.Secret{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-secret",
					Namespace: "default",
		...
		It("skips Kinds with non-matching group prefix", func() {
			secret := &corev1.Secret{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-secret",
					Namespace: "default",
					OwnerReferences: []metav1.OwnerReference{
						{
							// Same Kind name but different group
	...
	Context("when some owner references need migration and some don't", func() {
		It("only migrates the outdated ones", func() {
			secret := &corev1.Secret{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-secret",
					Namespace: "default",
			...
			Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(secret), updatedSecret)).To(Succeed())
			Expect(updatedSecret.OwnerReferences).To(HaveLen(2))
			// Machine should be migrated
			Expect(updatedSecret.OwnerReferences[0].APIVersion).To(Equal(clusterv1.GroupVersion.String()))
			// Cluster should remain unchanged
			Expect(updatedSecret.OwnerReferences[1].APIVersion).To(Equal(clusterv1.GroupVersion.String()))
		})
	})

	Context("when preserving other owner reference fields", func() {
		It("preserves UID, Name, Kind, Controller, and BlockOwnerDeletion", func() {
			controller := true
			...
			ref := updatedSecret.OwnerReferences[0]
			Expect(ref.APIVersion).To(Equal(clusterv1.GroupVersion.String()))
			Expect(ref.Kind).To(Equal("Machine"))
			Expect(ref.Name).To(Equal("my-machine-name"))
			Expect(ref.UID).To(Equal(types.UID("unique-id-12345")))
